...
class App(ctk.CTk):
    ...
    def add_session_widget(self, session_data):
        """NEW: Creates a frame with a session button and a delete button."""
    ...
    def delete_session_handler(self, session_id):
        ...
        if messagebox.askyesno("Confirm Deletion", f"Are you sure you want to permanently delete session '{hostname}'? This cannot be undone."):
            try:
                ...
                # If successful, remove the session from the GUI immediately
    ...
    def update_gui_with_sessions(self, server_sessions):
        """UPDATED: Handles the new widget structure and status colors."""
        ...
        for session_data in server_sessions:
            ...
            if sid not in self.sessions:
                ...
                # Update existing session data (like last_seen)
                self.sessions[sid] = session_data

            # Update button color based on activity (e.g., seen in the last 60 seconds)
    ...
    def update_content_view(self, selected_title, fallback_text=None):
        content = fallback_text if fallback_text else self.detail_view_data_map.get(selected_title, "Content not found.")
